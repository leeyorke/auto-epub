...
控制台只打印摘要（受 DEBUG_MODE 控制），细节一律进文件。
"""
...
from .settings import DEBUG_MODE, LOG_DIR, LOG_EXCERPT_CHARS, LOG_TO_FILE


class TranslationLogger:
    """把翻译过程的诊断信息同时输出到控制台和日志文件"""

    def __init__(self, book_name: str = "epub"):
    ...
    def chunks(self, index: int, chunk_stats: list[dict[str, int]]) -> None:
        ...
        self._console(f"    最大分块 {biggest} tokens，合计 {total_tokens} tokens")
...
def init_logger(book_name: str) -> TranslationLogger:
    """在一次翻译开始时初始化日志器"""
    global _logger
    _logger = TranslationLogger(book_name)
    return _logger
