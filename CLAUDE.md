...
2. **分块发放必须非破坏性**：只发放"还没有译文的块"，写入靠 `chunk_index` 定位，不用队列 pop
...
6. **每一次工具调用都要留一行日志**：工具的错误是 `return` 字符串而非 `raise ModelRetry`，pydantic-ai 视为成功，可以无限循环
7. **`INPUT_MAX_TOKENS` 必须显著小于 `OUTPUT_MAX_TOKENS`**（当前 5000 / 16384）
